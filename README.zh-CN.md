# <img src="https://raw.githubusercontent.com/gengxy1216/FlockMem/main/docs/assets/flockmem-icon.svg" alt="FlockMem logo" width="32" /> FlockMem（中文）
...
FlockMem 是一个面向 AI Agent 的本地优先长时记忆系统，也可作为多智能体协同的记忆基础设施。
...
FlockMem 不只是单个助手的“记忆库”，而是面向协作系统的记忆底层能力：
...
Windows 双击启动文件：`start_flockmem.bat`  
...
- Windows：`%LOCALAPPDATA%\\MiniMem`（为兼容旧版本保持不变）
...
仓库提供了可独立运行的 MCP 桥接能力，便于其他 Agent 直接集成 FlockMem：

- MCP 服务目录：`integrations/flockmem-mcp/server.py`
- Skill 目录：`skills/flockmem-mcp-integration`
...
FlockMem 使用分层记忆架构，在简单性、召回质量和检索性能之间做平衡：
...
- API：`docs/flockmem/api-reference.md`
- 架构：`docs/flockmem/architecture.md`
- 部署档位：`docs/flockmem/deployment-profiles.md`
- 运行时策略：`docs/flockmem/runtime-control.md`
- 冲突策略：`docs/flockmem/conflict-strategy.md`
- 评测：`docs/flockmem/evaluation.md`
- 使用文档：`docs/flockmem/user-guide.md`
